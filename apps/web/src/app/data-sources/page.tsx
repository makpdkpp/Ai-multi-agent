...
import { DataSourceManager, DataSourceRecord } from "@/components/data-source-manager";
...
async function loadPage(): Promise<{
  ...
  const [agentGroups, sourceGroups] = await Promise.all([
    Promise.all(
    ...
    Promise.all(
      departments.map(async (department) => {
        const response = await apiFetch(`/departments/${department.id}/data-sources`);
        if (!response.ok) throw new Error("Unable to load data sources");
        return (await response.json()).data as DataSourceRecord[];
      }),
    ),
  ]);

  const canManageSources = user.system_role === "super_admin"
    ...
    <main className="shell">
      <aside className="sidebar">
        ...
        <nav aria-label="เมนูหลัก">
          ...
          <Link className="navItem active" href="/data-sources">Data Sources</Link>
        ...
        <div className="sidebarFooter">
          ...
          <div><strong>{data.user.display_name}</strong><small>Data Sources</small></div>
        </div>
      </aside>
      <section className="content">
        <header className="topbar">
          <span>AgentDesk / <strong>Data Sources</strong></span>
        ...
        <div className="page">
          <DataSourceManager
            ...
            canManageSources={data.canManageSources}
          />
        </div>
      </section>
    </main>
